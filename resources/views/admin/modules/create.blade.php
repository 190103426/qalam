...
@section('title','Админ панель - Модуль қосу')
@section('custom_css')
    <link rel="stylesheet" href="{{asset('admin_asset/admin.css')}}">
@endsection
@section('content')
    @component('admin.components.breadcrumb')
        @slot('title') Модуль қосу @endslot
        @slot('parent') Басты бет @endslot
        @slot('courses') Курстар тізімі @endslot
        @slot('course_modules') {{ $course->name }} @endslot
        @slot('course_module_route') {{ route('admin.modules.index', ['course' => $course->id]) }} @endslot
        @slot('active') Модуль қосу @endslot
    @endcomponent
                ...
                <div class="col-lg-12">
                    <div class="card card-primary">
                        <form method="post" action="{{ route('admin.modules.store',
                                          ['course' => $course->id])}}"
                              class="add_product_form" name="add_product_form"
                        >
                            @csrf
                            @method('POST')
                            <div class="card-body">
